...
/** Board Class to be used in conjunction with Squatter class.
...
public class Board {
	...
	public Board() {
		...
		try {
			boardDims = Integer.parseInt(input.nextLine());
		...
		for(int row = 0; row < (board.getBoardDims()-1); row++){
			for(int col = 0; col < (board.getBoardDims()-1); col++){
				...
				if (board.getCells()[row][col].equals("-")){
					...
					//Add to tally
					if(lastCol.equals("B")){
						Squatter.setTallyB(Squatter.getTallyB() + 1);
					} else {
						Squatter.setTallyW(Squatter.getTallyW() + 1);
					}
					
					//debug
					if(debug){System.out.println("TallyW: "+Squatter.getTallyW()+", TallyB: "+Squatter.getTallyB());}
